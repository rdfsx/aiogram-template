...
class MongoStorage(BaseStorage):
    ...
    async def set_state(
        ...
        expiration = (
            datetime.utcnow() + timedelta(seconds=self.state_ttl) if self.state_ttl else None
        )
...
class DefaultMongoLock:
    ...
    async def touch(self, timeout: float = DEFAULT_MONGO_LOCK) -> None:
        ...
        await self.collection.update_one(filter=self.key, update={"$set": {"expireAt": expire}})
